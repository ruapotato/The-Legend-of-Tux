...
const BIOME_RINGS: Array = [
	{
		...
		"foliage": [
			...
			# Wood-bushes are the early-game bare-fist wood source —
			# trees take a sword/axe so the player needs an entry
			# rung. Heavily weighted in the starter biome.
			# (Dungeon-style bush.tscn removed from the survival
			# biome: it one-shotted and dropped pebbles/hearts, so it
			# looked identical to wood_bush but didn't give wood.)
			{"scene": "res://scenes/wood_bush.tscn",     "weight": 8.0},
