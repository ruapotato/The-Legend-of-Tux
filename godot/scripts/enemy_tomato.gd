...
func _physics_process(delta: float) -> void:
    ...
    # Always spin the visual on its Y axis at the current spin speed.
    ...
    match state:
        ...
        State.HOVER:
            ...
            _hover_to(HOVER_HEIGHT, delta)
            # Drift slowly toward the player so it doesn't feel inert.
            if dist > 0.5 and dist < aggro_range * 1.5:
            ...
            # Commit to a charge after a short wind-up.
            if state_time > 1.0 and dist < aggro_range:
